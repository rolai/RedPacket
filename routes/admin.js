...
router.get('/query-user', function(req, res, next) {
    res.render('admin/query-user.ejs', {
        message: ''
    });
});
...
router.post('/query-user', function(req, res, next) {
    ...
    query.find()
        .then(function(rows) {
            ...
            res.render('admin/query-result.ejs', {
                users: users
            });
        }, function(err) {
            console.log(err);
...
router.post('/charge/:userId', function(req, res, next) {
    ...
    if (money > 0) {
        ...
        utils.charge(req.currentUser, req.params.userId, money)
            .then(function(result) {
                return query.get(req.params.userId)
                    .then(function(row) {
                        var user = utils.userSummary(row);
                        var message;
                        ...
                        res.render('admin/charge-result.ejs', {
